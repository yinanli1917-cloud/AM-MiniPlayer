...
- ❌ (2026-09-21) Drawing the same lyric text with TWO text engines across a state change (CATextLayer whole-line base for inactive rows vs NSLayoutManager per-run bitmaps for the active line) → Core Text and NSLayoutManager place wrapped lines at different pitch/first-line offset, so the CATextLayer→bitmap swap on the activation tick moved a wrapped row's first line −3px and second line −6px on device (headless: −0.48/−0.95) — the "切行前入场行抖一下" the founder chased for a day. Blur/wave/rasterization/float toggles were all ruled out live before the probe (`row_geom`, switch-window ring buffer) proved the swap was the only change that tick.
  ✅ ONE layout engine for both states: `NativeLyricsLayoutTextLayer` draws the whole-line base through the same NSLayoutManager recipe as `NativeLyricsActiveLineDrawLayer.runImage` (same attributes, container, lineFragmentPadding 0, font smoothing/subpixel settings). Pinned by `NativeLyricsIncomingRowGeometryTests.test_activationDoesNotJumpTransformedInk_wrappedRows` (the incoming row MUST wrap and the sampler must map the row's own transform into surface space — the earlier samplers were blind to both).
- ❌ Advancing the presentation springs by the raw wall-clock gap between display-link callbacks → callback jitter becomes uneven per-frame displacement (−5,−5,−6,−3,−8,−6 … +2 in one smooth wave; founder: "动一动卡一卡").
  ✅ `NativeLyricsFrameStep.quantizedDelta`: step by whole display intervals (round(raw/nominal), 1…4).
- ❌ Writing a per-frame compensated value (`applyDimBaseCompensation`) only to the HIDDEN whole-line base while the visible single-pass draw layer keeps the value frozen from activation → the receding row reads too dark, then pops to the right brightness when `finalizeDeferredDeactivation` swaps layers.
  ✅ Mirror the compensated dim alpha onto the draw layer (`setDimAlpha`) whenever it is the visible one; pinned by `NativeLyricsDeferredDeactivationBrightnessTests`.
- ❌ (2026-09-21) Hiding the single-pass bitmaps and showing the whole-line base at the deactivation tick while the sung words were still held at their −2pt float → the history row drops 1.9px in one frame (啟程 '才找到永恒'). ✅ `leaveSinglePassActiveLine` carries `currentHeldFloat` onto the base layers and `advanceFloatReturn` eases it to 0 over 0.35s (contract). Pinned by the real-song sweep `test_realSong_qicheng_noInkJumpAtAnySwitch` + `test_realSong_qicheng_outgoingRowFloatAtSwitch7`.
- ❌ Baking `minimumLineHeight == maximumLineHeight` (and lineSpacing 0) into the base string's paragraph style "to match the bitmaps" → the moment a shared `mainLineSpacing` exists it silently overrides it and shifts the whole base block by one spacing (Δ−3.8 at the swap). One paragraph-style recipe, no per-path overrides.
- ❌ Leaving a diagnostic probe armed in the founder's preferences, or arming an expensive per-tick probe (5 rows × `layer.presentation()` + string formatting) during feel evaluation → switch ticks went 2–3ms → 5–7.5ms and dropped frames at wave onset, which the founder then reported as a new stutter. Probes are per-process cached: flipping the pref needs a relaunch.
