...
final class RapidSwitchTests: XCTestCase {
    ...
    func testNoCurrentTrackQueueSnapshotClearsRecentRowsEvenWhenRecentFetchIsThrottled() {
        ...
        c.recentRawRowCount = 1
        c.lastRecentHistoryFetchAt = Date()
        c.upNextProvenance = .exactPublicMusicQueue(context: "verified-before-no-current-snapshot")
        c.recentTracksProvenance = .exactPublicMusicQueue(context: "verified-before-no-current-snapshot")

        XCTAssertTrue(c.applyNoCurrentTrackQueueSnapshotIfNeeded(.unavailable(reason: .noCurrentTrack)))

        XCTAssertEqual(c.queueSyncGeneration, 12)
        XCTAssertNil(c.currentPersistentID)
        XCTAssertTrue(c.upNextTracks.isEmpty)
        XCTAssertTrue(c.recentTracks.isEmpty)
        XCTAssertEqual(c.upNextRawRowCount, 0)
    ...
    func testNoCurrentTrackRecentSnapshotClearsUpNextRowsEvenWhenUpNextFetchIsDelayed() {
        let c = MusicController(preview: true)
        c.isPreview = false
        c.queueSyncGeneration = 21
        c.currentPersistentID = "old-track"
        c.upNextTracks = [
            (title: "Old Next", artist: "Artist", album: "Album", persistentID: "next", duration: 180)
        ]
        ...
        c.recentRawRowCount = 1
        c.lastRecentHistoryFetchAt = Date()
        c.upNextProvenance = .exactPublicMusicQueue(context: "verified-before-recent-no-current-snapshot")
        c.recentTracksProvenance = .exactPublicMusicQueue(context: "verified-before-recent-no-current-snapshot")

        XCTAssertTrue(c.applyNoCurrentTrackQueueSnapshotIfNeeded(.unavailable(reason: .noCurrentTrack)))

        XCTAssertEqual(c.queueSyncGeneration, 22)
        XCTAssertTrue(c.upNextTracks.isEmpty)
        XCTAssertTrue(c.recentTracks.isEmpty)
        XCTAssertEqual(c.upNextRawRowCount, 0)
        XCTAssertEqual(c.recentRawRowCount, 0)
    ...
    func testMusicAppUnavailableSnapshotClearsBothQueueSurfaces() {
        let c = MusicController(preview: true)
        c.isPreview = false
        c.queueFetchPending = true
        c.queueFetchPendingForceRecent = true
        c.queueFetchPendingQueueGeneration = 31
        c.queueFetchPendingTrackGeneration = 5
        c.queueSyncGeneration = 31
        c.currentPersistentID = "old-track"
        c.currentTrackIsURLTrack = true
        c.upNextTracks = [
            (title: "Old Next", artist: "Artist", album: "Album", persistentID: "next", duration: 180)
        ...
        XCTAssertEqual(c.queueSyncGeneration, 32)
        XCTAssertFalse(c.queueFetchPending)
        XCTAssertFalse(c.queueFetchPendingForceRecent)
        XCTAssertNil(c.queueFetchPendingQueueGeneration)
        XCTAssertNil(c.queueFetchPendingTrackGeneration)
    ...
    func testQueueHashProbeUnavailableClearsStaleRowsAndResetsHashBaseline() {
        ...
        c.recentRawRowCount = 1
        c.lastRecentHistoryFetchAt = Date()
        c.upNextProvenance = .exactPublicMusicQueue(context: "verified-before-hash-probe-loss")
        c.recentTracksProvenance = .exactPublicMusicQueue(context: "verified-before-hash-probe-loss")
        let unavailableGeneration = c.queueSyncGeneration

        XCTAssertTrue(c.applyQueueHashProbeUnavailable())

        XCTAssertEqual(c.queueSyncGeneration, unavailableGeneration + 1)
        XCTAssertFalse(c.queueFetchPending)
        XCTAssertFalse(c.queueFetchPendingForceRecent)
        XCTAssertNil(c.queueFetchPendingQueueGeneration)
        XCTAssertNil(c.queueFetchPendingTrackGeneration)
        ...
        XCTAssertTrue(c.applyDetectedQueueHash("Music:10:current"))
        XCTAssertEqual(c.queueSyncGeneration, unavailableGeneration + 2)
        XCTAssertEqual(c.upNextProvenance, .unavailable(reason: .pendingPublicRefresh))
        XCTAssertEqual(c.recentTracksProvenance, .unavailable(reason: .pendingPublicRefresh))
    }
    ...
    func testQueueHashProbeMissingCurrentPlaylistClearsRowsWithoutClearingTrackIdentity() {
        let c = MusicController(preview: true)
        c.isPreview = false
        XCTAssertTrue(c.applyDetectedQueueHash("Music:10:current"))
        c.queueFetchPending = true
        ...
        c.recentRawRowCount = 1
        c.lastRecentHistoryFetchAt = Date()
        c.upNextProvenance = .exactPublicMusicQueue(context: "verified-before-missing-playlist")
        c.recentTracksProvenance = .exactPublicMusicQueue(context: "verified-before-missing-playlist")
        let unavailableGeneration = c.queueSyncGeneration

        XCTAssertTrue(c.applyQueueHashProbePublicStateUnavailable(reason: .noCurrentPlaylistForTrackClass("URL track")))
        ...
        XCTAssertNil(c.queueFetchPendingTrackGeneration)
        XCTAssertEqual(c.currentPersistentID, "old-track")
        XCTAssertTrue(c.currentTrackIsURLTrack)
        XCTAssertTrue(c.upNextTracks.isEmpty)
        XCTAssertTrue(c.recentTracks.isEmpty)
        XCTAssertEqual(c.upNextRawRowCount, 0)
        XCTAssertEqual(c.recentRawRowCount, 0)
        XCTAssertEqual(c.lastRecentHistoryFetchAt, .distantPast)
        XCTAssertEqual(c.upNextProvenance, .unavailable(reason: .noCurrentPlaylistForTrackClass("URL track")))
        XCTAssertEqual(c.recentTracksProvenance, .unavailable(reason: .noCurrentPlaylistForTrackClass("URL track")))
        XCTAssertFalse(MusicController.shouldApplyQueueSnapshot(
            requestQueueGeneration: unavailableGeneration,
            currentQueueGeneration: c.queueSyncGeneration,
            requestTrackGeneration: 6,
            currentTrackGeneration: 6
        ))
    ...
    func testRepeatedQueueHashProbePublicStateUnavailableDoesNotChurnAfterSettled() {
        let c = MusicController(preview: true)
        c.isPreview = false
    ...
    func testNonWholeQueueUnavailableSnapshotDoesNotClearRowsThroughWholeQueuePath() {
        ...
        c.upNextProvenance = .exactPublicMusicQueue(context: "verified-before-source-gap")
        c.recentTracksProvenance = .exactPublicMusicQueue(context: "verified-before-source-gap")

        XCTAssertFalse(c.applyWholeQueueUnavailableSnapshotIfNeeded(.unavailable(reason: .publicSourceUnverified)))

        XCTAssertEqual(c.queueSyncGeneration, 41)
        XCTAssertEqual(c.upNextTracks.map { $0.persistentID }, ["next"])
        XCTAssertEqual(c.recentTracks.map { $0.persistentID }, ["recent"])
        XCTAssertEqual(c.upNextProvenance, .exactPublicMusicQueue(context: "verified-before-source-gap"))
        XCTAssertEqual(c.recentTracksProvenance, .exactPublicMusicQueue(context: "verified-before-source-gap"))
    }

    func testNonNoCurrentQueueSnapshotDoesNotClearRowsThroughNoCurrentPath() {
        let c = MusicController(preview: true)
        c.isPreview = false
        c.queueSyncGeneration = 11
        c.upNextTracks = [
        ...
        c.upNextProvenance = .exactPublicMusicQueue(context: "verified-before-partial-snapshot")

        XCTAssertFalse(c.applyNoCurrentTrackQueueSnapshotIfNeeded(.unavailable(reason: .publicSourceUnverified)))

        XCTAssertEqual(c.queueSyncGeneration, 11)
        XCTAssertEqual(c.upNextTracks.map { $0.persistentID }, ["next"])
        XCTAssertEqual(c.upNextProvenance, .exactPublicMusicQueue(context: "verified-before-partial-snapshot"))
    }
