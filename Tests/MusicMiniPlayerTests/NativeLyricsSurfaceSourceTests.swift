...
final class NativeLyricsSurfaceSourceTests: XCTestCase {
    ...
    func testNativeSurfaceDoesNotHostSwiftUIRowViews() throws {
        ...
        XCTAssertTrue(source.contains("NSEvent.addLocalMonitorForEvents(matching: [.scrollWheel, .mouseMoved])"))
        XCTAssertTrue(source.contains("Tap-to-jump is handled by the"))
        ...
        XCTAssertTrue(source.contains("private var textActiveByRowIndex: [Int: Bool]"))
        XCTAssertTrue(source.contains("var nativeTextActiveIndex: Int? = nil"))
        XCTAssertTrue(source.contains("var effectiveTextActiveIndex: Int"))
        XCTAssertTrue(source.contains("private func configurationForTextPhase("))
        XCTAssertTrue(source.contains("private func shouldDriveTextPhase("))
        XCTAssertTrue(source.contains("refreshTextActivation(runtimeConfiguration: runtimeConfiguration)"))
        XCTAssertTrue(source.contains("beginDeferredDeactivation(index: row.index"))
        XCTAssertTrue(source.contains("textConfiguration.effectiveTextActiveIndex == runtimeConfiguration.effectiveCurrentIndex"))
        XCTAssertTrue(source.contains("guard state.opacity < 0.38 else { return }"))
        XCTAssertTrue(source.contains("applyFrames(runtimeConfiguration: runtimeConfiguration, snap: false, managesTransaction: false)"))
        XCTAssertFalse(source.contains("let needsActivationRefresh = textPhaseIndexChanged"))
        XCTAssertFalse(source.contains("row.index == activeTextPhaseIndex"))
        XCTAssertFalse(source.contains("private func refreshTextRowsForActiveLineChange("))
        XCTAssertFalse(source.contains("if activeTextLineChanged {\n                refreshTextRowsForActiveLineChange"))
