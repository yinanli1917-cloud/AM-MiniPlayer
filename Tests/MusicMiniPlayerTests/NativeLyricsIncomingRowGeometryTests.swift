...
final class NativeLyricsIncomingRowGeometryTests: XCTestCase {
    ...
    func test_incomingRowInkGeometryAcrossHandoff_namesTheTwitchChannel() {
        ...
        report += "the whole-line dim base (`mainTextLayer`) to the bitmap single-pass active-line "
        report += "renderer (`NativeLyricsActiveLineDrawLayer`, `applySinglePassActiveLine`). The row's own "
        report += "FRAME origin (`frameY`) does not move that tick.\n\n"
        report += "CIFilters (the stepped blur, springed-blur v28 arm) cannot run under headless "
        report += "`layer.render(in:)`, so this measurement cannot see or rule out a blur-only contribution — "
        report += "orthogonal to the fix below, which is about WHERE the ink is before blur ever runs.\n\n"
        report += "## Fixed / left failing\n\n"
        report += "Fixed (2026-09-21, `NativeLyricsLayoutTextLayer`). The named channel was that `mainTextLayer` "
        report += "drew its whole-line string via CATextLayer/Core Text (CTFramesetter) while "
        report += "`NativeLyricsActiveLineDrawLayer` laid the SAME text out via NSLayoutManager/TextKit — two "
        report += "materially different engines with no documented sub-pixel rounding contract between them, so "
        report += "wrapped CJK lines pitched a fraction of a point apart. Rather than reconciling two engines' "
        report += "rounding after the fact, `mainTextLayer`/`mainBrightTextLayer` are now a CALayer subclass "
        report += "(`NativeLyricsLayoutTextLayer`, Sources/MusicMiniPlayerCore/UI/NativeLyricsLayerSupport.swift) "
        report += "that lays out and draws through the identical NSLayoutManager/NSTextContainer recipe "
        report += "(`lineFragmentPadding = 0`, unbounded height, word-wrap by the layer's own bounds width) and "
        report += "font-smoothing/subpixel-positioning context settings `NativeLyricsActiveLineDrawLayer.runImage` "
        report += "uses — one engine, so both rasterizers agree by construction. "
        report += "`NativeLyricsIncomingRowGeometryTests.test_incomingRowInkGeometryAcrossHandoff_namesTheTwitchChannel` "
        report += "and `test_activationDoesNotJumpTransformedInk_wrappedRows` both pass; the prior "
        report += "`withFixedWrapLineHeight` hand-tuned-line-height attempt (which did not move these pins at "
        report += "all) is left in place as a harmless no-op for single-line text.\n"
