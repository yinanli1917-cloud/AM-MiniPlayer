...
def toc(trace_path: Path) -> TraceInfo:
    """Export the trace's table of contents and return schemas + metadata.
    ...
    schemas: set[str] = set()
    for table in root.iterfind(".//data/table"):
        schema = table.get("schema")
        if schema:
            schemas.add(schema)

    template = _find_text(root, ".//summary/template-name")
    duration = _find_text(root, ".//summary/duration")
    start = _find_text(root, ".//summary/start-date")
    end = _find_text(root, ".//summary/end-date")
    instruments = _find_text(root, ".//summary/instruments-version") or ""
...
def export_schema(trace_path: Path, schema: str, run: int = 1) -> bytes:
    """Export one schema's data as XML bytes.
