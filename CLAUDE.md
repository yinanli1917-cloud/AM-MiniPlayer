...
**歌词源**（7个，并行请求）：
1. **AMLL-TTML-DB** (+10) - 逐字时间轴，最高质量
2. **NetEase 网易云** (+8) - 中文歌首选，支持 YRC 逐字 + 翻译
3. **QQ Music** (+6) - 中文歌第二选择，支持翻译
4. **SimpMusic** (+5) - 全球化歌词源，基于 YouTube Music 社区
5. **LRCLIB** (+3) - 开源歌词库，精确匹配端点
6. **LRCLIB-Search** (+2) - LRCLIB 模糊搜索端点
7. **lyrics.ovh** (+0) - 纯文本备选

**多区域元信息获取**：
- 解决"系统语言英文 → iTunes 返回英文标题 → 中文歌词库无法匹配"问题
- 自动检测日/韩/泰/越文字符，查询对应区域 iTunes API
- 支持罗马字艺术家名的启发式区域推断

**匹配算法**：
- 时长匹配（40%权重）+ 标题相似度（35%）+ 艺术家匹配（25%）
- 标题/艺术家规范化：移除 (feat.), [Remaster], (Live) 等后缀
- 质量阈值：评分 >= 50 才接受
