/**
 * [INPUT]: transition kind, whether it starts tucked, geometry, bounce.
 * [OUTPUT]: LiquidEdgeChoreography.stages(...) — key poses and springs.
 * [POS]: Liquid edge (ported from the approved prototype, 2026-09-22).
 * [PROTOCOL]: Every geometry channel moves on ONE spring per transition,
 *             launched at speed (exponential ease-out); the shape story comes
 *             from axes running at different rates (drop: width fast, height
 *             1.7x slower; collapse: height 0.32s vs width 0.44s — the swept
 *             pair giving height-first AND a stalk; expand: width faster).
 *             Only non-geometry channels, the landing dive-and-rebound and
 *             the sliver's drain may take extra stages.
 */

import Foundation

public enum LiquidEdgeChoreography {
    typealias S = LiquidEdgePlan.Step
    typealias Stage = LiquidEdgeMotion.Stage
    typealias G = LiquidEdgeChannelGroup
    ...
    public static func stages(kind: LiquidEdgeTransition, fromTucked: Bool, geometry: LiquidEdgeGeometry,
                              bouncy: Bool = true) -> [LiquidEdgeMotion.Stage] {
        let poses = LiquidEdgePoses(geometry)
        func s(_ d: Double, _ b: Double = 0, _ delay: Double = 0, impulse: Double? = nil) -> S {
            LiquidEdgePlan.step(d, b, delay: delay, impulse: impulse)
        }
        func pose(_ p: LiquidEdgeKeyPose) -> LiquidEdgePose { poses.pose(p) }
        func stage(_ at: Double, _ to: LiquidEdgePose, _ steps: [G: S], impulse: Double) -> Stage {
            Stage(start: at, to: to.vector(), plan: LiquidEdgePlan(steps, fallback: s(0.2)), impulse: impulse)
        }
        let cardEnd = poses.cardFromCapsule
    ...
    public static func direct(to: [Double]) -> [LiquidEdgeMotion.Stage] {
        func s(_ d: Double, _ b: Double = 0) -> LiquidEdgePlan.Step { LiquidEdgePlan.step(d, b) }
        return [Stage(start: 0, to: to, plan: LiquidEdgePlan(
