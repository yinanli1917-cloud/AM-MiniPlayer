...
| 3d | Round 2 (user eyes-on 2026-07-12): recede before interludes read as "sinking + fogging"; dots reached the centre with a ~3s delay | §B/§C | **Fixed 2026-07-12 round 2.** Recede = dim + shrink ONLY (blur landing 0 in both target variants); interlude blend ramp 0.5s+2.5s → 0.1s+0.6s (a normal line-handoff feel; the position spring smooths on top); the ordinary-gap fold keeps its own gentle 2.5s (`gapRecedeDuration`). |
