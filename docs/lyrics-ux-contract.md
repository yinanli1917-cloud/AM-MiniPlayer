...
| 🔴 long-line segmentation **display-only**; CJK never re-spaced; no orphan-balancing hacks; **superseded 2026-09-22 (Plan A, founder-approved)**: the split trigger is the renderer's own real NSLayoutManager wrap at the current column width, not a unit-count estimate, so the old "≈8-word phrases stay one unit" heuristic is gone — a piece splits whenever it would exceed 2 real visual lines, full stop, no phrase-length exemption. Every piece gets its OWN translation (**superseded again, same day, Phase 2**: not "every chunk gets [the same] translation" — see below) | ✅ landed, `LongLineEvalTests` / `LyricPieceTranslationTests` |
| Translation for split pieces (2026-09-22 Phase 2, founder decision — overrides the interim "translation on the first piece only"): three tiers per line — (1) clause-aligned pairing when the original was cut at clause punctuation and the translation splits into the same clause count *with the same relative clause-length order* (a conservative, honestly-imperfect guard against clause reordering across languages — rejects on ANY doubt, since a wrong positional pairing is worse than a blunt-but-correct fallback); (2) an async per-piece on-device translation, cached by (piece text, source, target); (3) if neither is available yet, the full translation attaches to the first piece only (never chopped by length). See `LyricPieceTranslation.swift` + `research/translation-popup-and-piece-translation-2026-09-22.md`. | ✅ landed |
...
| 3d | Round 2 (user eyes-on 2026-07-12): recede before interludes read as "sinking + fogging"; dots reached the centre with a ~3s delay | §B/§C | **Fixed 2026-07-12 round 2, REFINED 2026-07-13 round 3 (user).** TWO fold flavors, separate params on both target variants: INTERLUDE fold (dots take the centre → the row IS the previous dist-1 line) lands WITH the natural dist-1 depth blur; ORDINARY-GAP fold (row keeps the centre) is dim + shrink only, blur 0. Interlude blend ramp 0.1s+0.6s; gap fold keeps its gentle 2.5s (`gapRecedeDuration`). |
| 3h | 2026-09-20 founder: 和声升为数据模型（`LyricLine.isBackground`，来源 TTML x-bg / 整行括号 / 行首尾括号拆分），渲染为主行的从属行：0.8× 字号、亮度低一档（tier ladder 1.0/0.85/0.6/0.35/0.22）、与主行同时点亮、不放大、不作模糊焦点、行距减半、永不占滚动位。取代 3g 的文本启发式 `isBackingVocalText`。数值待创始人实机终验。 | §A sync | Implemented 2026-09-20 (`NativeLyricsBackgroundRowTests`). |
| 3g | Round 3 (user 2026-07-13): 和声应该同时播放 — backing-vocal lines (（I want you）) took the primary slot for their tiny sequential windows, yanking the scroll | §A sync | **Fixed 2026-07-13.** Structural rule (`isBackingVocalText`: whole line bracket-wrapped, （）/()): backing parts stay in hotGroups (light up at the harmony tier ALONGSIDE the melody) but never claim the primary slot — semanticIndex/scrollToIndex/liveDisplayIndex/firstFutureIndex resolve on melody lines, with all-backing fallbacks. Combined with 3f (zero-duration window rides its sibling): duet parts read simultaneous, scroll stays melody-to-melody. |
| 3e | Taps dead in the bottom controls region after the controls fade (worst during manual scroll) | §E interaction | **Root-caused + fixed 2026-07-12.** `configureSignature` omitted `controlsVisible`, so the "controls now hidden" reconfigure was deduped whenever nothing else changed (frozen index during manual scroll = exactly that) — the surface kept the stale `controlsVisible=true` and the bottom reserved zone kept eating taps. Field added to the signature; `NativeLyricsTapHitTests` (5 scenarios incl. the fade-out dedupe repro). |
| 3f | Harmony/backing-vocal stutter (Puzzle — Meiko Nakahara): scroll bounced up-down at「（I want you）」 | §A sync | **Root-caused + fixed 2026-07-12 (policy level).** NetEase flattens simultaneous duet parts into a ZERO-DURATION line co-starting with the real line (157.98-157.98). `effectiveEndTime`'s fallback (next DISTINCT start) let it OUTLIVE its sibling (ends 13.28-equiv), so in the 80ms hole it was the only hot row and `semanticIndex = hotGroups.max()` regressed to it → one-frame back-jump = the stutter. Fix: a zero-length line's window rides its co-starting sibling's real end (`min(siblingEnd, fallback)`). Pure-policy test green; the 1x drive test needs a QUIET machine to judge (ran at load avg 402 = noise). |
