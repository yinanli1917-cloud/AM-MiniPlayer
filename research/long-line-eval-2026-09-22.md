...
## 结果 · 修订（2026-09-22，创始人复审后二次修复，commit `9ec5f63` → 见下方新 commit）

创始人复审 `9ec5f63` 后指出两个问题，均已修复：

### 问题 1：验收 (a) 循环论证

旧写法把"对这个片段再跑一次 `realWrapPieces`，如果还是切不动"当作"不可再拆"的判据——这样任何真实的分词器漏洞都会自我免检（漏切的片段再跑一次当然还是漏切）。改为**独立于分词器**的判据：一个片段只有在满足"整段全是拉丁字母/数字、零空白、零标点"时才算不可再拆；任何 ≥2 字形的 CJK/假名/谚文连续段永远视为可再拆（不管分词器实际怎么处理）。

换上独立判据后，穷举整个 54 行数据集只挖出 **1 处真实分词器漏洞**：`gt-005`「想爱 就不能害怕会有伤痕」（原始 NetEase 数据里那个空格其实是不换行空格 U+00A0）——根因是孤字合并的安全网 `mergeOrphanTextPieces` 把"恰好等于下限字数（2字）"的片段也当孤字合并掉，和挑选断点时 `pick()` 自己"恰好 2 字是安全下限"的判断自相矛盾，切完立刻被缝回去。改为"严格小于下限才算孤字"，两处判据对齐，`gt-005` 现在正确切成两段。已用 `test_acceptance_a` 钉死（改动前会复现失败，改动后绿）。

### 问题 2：≥4 视觉行行位清单 + 合并策略改为"够 2 段就不再退到 1 段，2 段门槛降到 0.6s"

排查过程中额外挖出两个真实分词器/合并策略漏洞（用独立判据一暴露就现形，问题 1 和问题 2 其实是同一次穷举撞见的）：

1. **合并方向可能把两个本来合规的片段粘成新的超长片段**：`syn-008`「I keep running」+「through the」按字符权重选了"更轻的邻居"合并，糊成「I keep running through the」（4 行）——明明换个方向合并（跟后面「midnight looking」粘）也一样是 4 行，但两个方向都没检查合并后还符不符合 2 行预算。改为：先比较两个合并方向各自合并后的真实视觉行数，优先选择不超预算的方向；两边都不超预算或都超预算再按行数、再按字符权重找退而求其次的选择。
2. **合并会一路做到底，不管沿途造出多长的墙**：即便换了合并方向，有些相邻文本本来就没法拼在一起还留在 2 行以内（例如两个本来各自刚好 2 行的英文短语拼起来必然超）。创始人明确的"够 2 段就不再退到 1 段"原则，我把它推广成一条更细的规则：一个片段只是"软性"低于 1.2 秒下限（但仍 ≥ 0.6 秒）时，如果合并会造出超预算的墙，就**不合并**，宁可让它稍微短一点也不重新制造墙；只有真正低于 0.6 秒的硬下限片段才无条件强制合并（此时哪怕短暂当一堵墙也比一闪而过的 0.x 秒行更好）。新增 `LyricTimedPiece.durationBelowFloorToAvoidVisualWall` 字段忠实记录这个例外，供验收 (e) 精确豁免（不再需要整行打包豁免，逐段判断）。
3. 顺带修了一个只有在 emoji/CJK 混排文本上才会现形的分词器漏洞：紧凑文字系统字符边界断点（第 5 优先级）原本要求"整段必须全是紧凑文字系统"才生效，导致「天了💕💕💕生日快乐🥳🎂🍰愿你永远开心」这种 CJK 夹 emoji 的片段找不到任何断点（emoji 不算标点也不算空白也不算紧凑文字系统）。改为：只要两个相邻字符都是紧凑文字系统（不要求整段都是），中间就可以断——这才是创始人原话"任何 ≥2 字形的 CJK/假名/谚文连续段永远可再拆"的正确实现。

**验收 (a)-(f) 全绿（含独立判据）；`swift build` 通过。**

### ≥4 视觉行清单（这次修复后的最终状态，逐行列因）

| id | 文本前缀 | 视觉行数 | 原因 |
|---|---|---:|---|
| syn-002 | "Antidisestablishmentaria..." | 9（180pt）/ 5（250pt） | 不可再拆的单一拉丁 token（80 字符无标点无空白的合成压力测试，符合规格"允许超预算、永不切进词内部"） |
| syn-003 | "(ooh ooh ooh child thing..." | 11（180pt）/ 6（250pt） | 和声/背景行，规格第 6 条永不拆分 |
| syn-006 | "这一句歌词特别长但是它在这首歌里只被唱了很短很短..." | 8（180pt）/ 5（250pt） | 时长合并：38 个汉字压在 0.3 秒总时长里，即便退到 2 段每段也只有 ~0.15 秒，远低于 0.6 秒硬下限，只能整行不拆（合成压力测试，真实歌词罕见这么极端的字密度） |

三条都是规格明确允许的例外（不可再拆 token / 背景行永不拆 / 极端时长退化到不拆），不是分词器漏洞。**总数从修复前的 6/54（180pt）、11/54（250pt）降到 3/54（两个宽度一致）**——三条剩余的都是同一批合成压力测试用例，真实数据集里的 51 行全部落在 ≤3 行以内。

### 修订前后对比（180pt，54 行数据集）

| 指标 | 本轮修订前（`9ec5f63`） | 本轮修订后 |
|---|---:|---:|
| stillFourPlus（narrow） | 6/54 | **3/54** |
| avgMaxVisualLines（narrow） | 2.630 | **2.426** |
| 250pt stillFourPlus | 11/54 | **3/54** |
| 250pt avgMaxVisualLines | 3.333 | **3.296** |
| Ground-truth 计时误差中位数 | 0.142s | 0.142s（不变，问题 2 的合并策略只影响非 ground-truth 用例，未触发 ground-truth 子集的合并路径变化） |

### 两个预先存在的失败：用临时 detached worktree 复核（未用 git stash）

按要求换用 `git worktree add --detach <tmp> main`（不碰共享的 stash 栈）：在独立于本 worktree 的一个临时目录里检出当前 `main`（`7fc82b5`），只跑 `NativeLyricsSurfaceSourceTests` 一个类。结果：`testNativeLyricsLayoutInsetsMatchV28SwiftUIRenderer` 和 `testNativeSurfaceDoesNotHostSwiftUIRowViews` 在 main 原样上**确认失败**，失败断言行号（38-39、98+194）与本 worktree 完全一致——不是 Plan A 引入的回归，是 main 上先前某次改动遗留、尚未修的问题。验完立即 `git worktree remove --force` 清理了临时目录，没有残留。

安全核查（本轮）：`~/Library/Application Support/nanoPod/` 全部文件 mtime+size 在本轮所有 `swift test`/`swift build` 前后逐字节比对，零差异。
